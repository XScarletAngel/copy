...
<div class="row-fluid">
	...
	<div class="span12">
		...
		<?php $str = get_nav_name('/index.php/Dict/Srv/index'); ?>
		...
		<ul class="breadcrumb">
			...
			<li>
				>
					<?php echo get_bread_nav('/index.php/Dict/Srv/index');?>
			</li>
		...
		<!-- END PAGE TITLE & BREADCRUMB-->
				...
				<div class="wrap js-check-wrap">
					<form class="well form-search" method="post" action="<?php echo U('Srv/index');?>">
						<ul class="fromUl clearfix">
							<li>
								服务名称：
								<input type="text" id="kw" name="kw" style="width: 200px;" value="<?php echo ($_REQUEST['kw']); ?>" placeholder="请输入...">
							</li>
						</ul>
						<div class="frombtn">
						...
						<table class="table table-hover table-bordered table-list">
							<thead>
								<tr>
									...
									<th width="30%">服务说明</th>
									<th width="10%">服务状态</th>
									<th width="20%"><?php echo L('ACTIONS');?></th>
								</tr>
							</thead>
							<?php if(is_array($res["data"])): foreach($res["data"] as $key=>$vo): ?><tr>
			                    <td><?php echo ($vo["id"]); ?></td>
								<td><?php echo ($vo["srv_name"]); ?></td>
								<td>
									<?php if($vo["srv_type"] == 1 ): ?>线上服务<?php else: ?>线下服务<?php endif; ?>
								</td>
								<td><?php echo ($vo["srv_info"]); ?></td>
								<td><?php if($vo["srv_status"] == 1 ): ?>启用<?php else: ?>停用<?php endif; ?></td>

								<td>
									<a class="editRow" url="<?php echo U('Dict/Srv/edit',array('id'=>$vo['id']));?>" href="javascript:void(0);"><?php echo L('EDIT');?></a> |
									<?php if($vo["srv_status"] == 2 ): ?><a href="javascript:void(0);" id-value="<?php echo ($vo["id"]); ?>" srv_status="1" class="changeSt">
											启用
										</a>
									<?php else: ?>
										<a href="javascript:void(0);" id-value="<?php echo ($vo["id"]); ?>" srv_status="2" class="changeSt">
											停用
										</a><?php endif; ?>
								</td>
							</tr><?php endforeach; endif; ?>
						</table>
						<div class="page clearfix"><?php echo ($res["page"]); ?></div>
				</div>
				<!-- 右边主页面结束 -->
				<script>
					$("#addSrv").on('click',function () {
						var html = "<?php echo U('Dict/Srv/add');?>";
						layer.open({
							type: 2,
							title: '添加服务',
				...
				<script>
					$(function(){
						$(".changeSt").click(function(){
							...
							var url = "<?php echo U('Dict/Srv/changeStatus');?>";
