...
class AdminbaseController extends AppframeController {
	...
	function _initialize(){
    	...
    	if(!empty($session_admin_id)){
			...
			if($user['pw_reset'] == 0 && 'admin/setting/password' != $path && 'admin/setting/password_post' != $path){
                $this->redirect('admin/setting/password');
            }
...
//	     }
	...
	// 公共权限变量方法
	public function load_menu_lang(){
	 ...
	 * @param array $data
	 */
	protected function ajaxJson($code, $msg, $data = [])
	{
		$ret['msg']   =   $msg;
		$ret['code']  =   $code;
		$ret['data']  =   is_array($data) ? $data : [];
	 ...
	 * @param $data
	 */
	protected function ajaxOk($msg = 'success', $data)
	{
		$this->ajaxJson(1, $msg, $data);
	}
	 ...
	 * @param $data
	 */
	protected function ajaxError($msg = '参数错误', $data = [])
	{
		$this->ajaxJson(0, $msg, $data);
	}
	...
	public function changeStatus()
	{
		...
		$upd_st = M($table)->where(['id'=>$id])->save([$key=>$value]);
		if (false !== $upd_st) {
			$this->ajaxOk("success");
		} else {
			$this->ajaxError("failed");
		}
	}
