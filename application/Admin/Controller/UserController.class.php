...
class UserController extends AdminbaseController{
	...
	public function _initialize() {
		parent::_initialize();
		$this->users_model = D("Common/AdminUsers");
	...
	public function add(){
        ...
        $data      = M("school_zone")->where("is_del = 1")->order("parentid desc")->select();
	...
	public function add_post(){
		if(IS_POST){
			if(!empty($_POST['role_id']) && is_array($_POST['role_id'])){
			    if(!empty($_POST['zone_id']) && is_array($_POST['zone_id'])){
                    ...
                    if ($this->users_model->create()!==false) {
                        ...
                        if ($result!==false) {
                            ...
                            $adminuser_zone_model = M("adminuser_zone");
                            // 添加角色
                            ...
                            foreach ($zone_ids as $v){
                                $adminuser_zone_model->add(array("zone_id"=>$v,"user_id"=>$result));
                            }
	...
	public function edit(){
        ...
        $categorys = $this->noLimitCate($data, $parentid=0, $level=0);
        $adminuser_zone_model=M("adminuser_zone");
        $zone_ids=$adminuser_zone_model->where(array("user_id"=>$id))->getField("zone_id",true);

        $this->assign("zone_ids",$zone_ids);
        $this->assign('categorys', $categorys);
		$this->assign($user);
		$this->display();
	}
                ...
                if(!empty($_POST['zone_id']) && is_array($_POST['zone_id'])) {
                    ...
                    if ($this->users_model->create() !== false) {
                        ...
                        if ($result !== false) {
                            ...
                            $adminuser_zone_model->where(array("user_id" => $uid))->delete();
                            foreach ($role_ids as $role_id) {
                                if (sp_get_current_admin_id() != 1 && $role_id == 1) {
                                    $this->error("为了网站的安全，非网站创建者不可创建超级管理员！");
                                }
                            ...
                            // 添加所属部门
                            foreach ($zone_ids as $v){
                                $adminuser_zone_model->add(array("zone_id"=>$v,"user_id"=>$uid));
                            }
                            $this->success("保存成功！");
                        } else {
                            $this->error("保存失败！");
