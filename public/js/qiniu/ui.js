...
// 用plupload插件的方法获取到文件的大小
        ...
        var progressBar = $("<div/>");
// 设置进度条属性，此处运用bootstrap属性，aria-valuemax设置进度条最大值最小值和当前值
...
//设置取消操作
        ...
        progressCancel.show().addClass('progressCancel btn btn-default _cancelbtn').text('取消');
...
FileProgress.prototype.setChunkProgess = function(chunk_size) {
    ...
    var viewProgess = $('<button class="btn btn-default _block">查看分块上传进度</button>');
...
FileProgress.prototype.setComplete = function(up, info) {
    ...
    var imageView = '?imageView2/1/w/50/h/50';
    ...
    var showImg = $('');//获取加载图片的dom

    progressNameTd.append(Wrapper);

    if (!isImg) {
        showImg.attr('src', '');//修改显示图片为默认图片
        ...
        $(img).on('load', function() {
            ...
            fopLink.on('click', function() {
                ...
                $('#myModal-img').on('hide.bs.modal', function() {
                    $('#myModal-img').find('.btn-default ._block').removeClass('disabled');
...
FileProgress.prototype.setCancelled = function(manual) {
    ...
    this.fileProgressWrapper.find('td:eq(2) .btn-default _block').hide();
...
FileProgress.prototype.bindUploadCancel = function(up) {
    ...
    if (up) {
        self.fileProgressWrapper.find('td:eq(2) .progressCancel').on('click', function(){
            ...
            self.setStatus("取消上传");
            self.fileProgressWrapper.find('.status').css('left', '0').css('margin-left','0');
